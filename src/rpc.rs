...
impl Rpc {
    pub fn new(
        ...
        nexus: &Nexus,
        rpc_id: u8,
        sm_handler: extern "C" fn(c_int, ffi::SmEventType, ffi::SmErrType, *mut c_void),
        phy_port: u8,
    ) -> Self {
        let rpc =
            unsafe { ffi::erpc_rpc_new(nexus.inner, context.inner, rpc_id, sm_handler, phy_port) };
        Rpc {
            inner: rpc,
            owner: true,
        }
    ...
    pub fn enqueue_response(&self, req_handle: &ReqHandle, data: Vec<u8>) -> () {
        unsafe {
            ffi::erpc_enqueue_response(self.inner, req_handle.inner, data.as_ptr(), data.len())
        }
    }
}
